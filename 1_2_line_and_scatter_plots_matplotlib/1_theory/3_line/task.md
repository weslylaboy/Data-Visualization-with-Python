## Theory

This visualization is much better, as we can now identify the big dense cluster in our dataset. Moreover, we can observe
a potential linear relationship between the two scores. To explore this further, let's attempt a linear approximation of the
data.

As mentioned before, `plot` can help us to build a linear plot. So let's try to apply it here.

## Task

Replace the `scatter` call with `plot`.

Note that you don't need to adjust the transparency or change the color for this plot.

## Hints

<div class="hint" title="What should the figure look like?">
   <img src="example.png" alt="What the figure should look like" style="max-height: 500px">
</div>
